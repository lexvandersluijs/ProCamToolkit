...
int glhProjectf(float objx, float objy, float objz, double *modelview, double *projection, int *viewport, float& windowCoordinateX, float& windowCoordinateY, float& windowCoordinateZ)
  {
...
int glhProjectToViewport(float objx, float objy, float objz, double *modelview, double *projection, int *viewport, float& windowCoordinateX, float& windowCoordinateY, float& windowCoordinateZ)
  {
      //Transformation vectors
      double fTempo[8];
      //Modelview transform
...
ofVec3f ofWorldToScreen(ofVec3f world) {
	...
	glhProjectf(world.x, world.y, world.z, modelviewMatrix, projectionMatrix, viewport, x, y, z);
...
ofVec3f ofWorldToViewport(ofVec3f world) {
	...
	GLfloat x, y, z;
	//gluProject(world.x, world.y, world.z, modelviewMatrix, projectionMatrix, viewport, &x, &y, &z);
	glhProjectToViewport(world.x, world.y, world.z, modelviewMatrix, projectionMatrix, viewport, x, y, z);
	
	ofVec3f inViewport(x, y, z);
	inViewport.y = viewport[3] - inViewport.y;
	return inViewport;
}
...
ofMesh getProjectedInViewportMesh(const ofMesh& mesh) {
	ofMesh projected = mesh;
	for(int i = 0; i < mesh.getNumVertices(); i++) {
		ofVec3f cur = ofWorldToViewport(mesh.getVerticesPointer()[i]);
		cur.z = 0;
		projected.setVertex(i, cur);
	}
	return projected;
